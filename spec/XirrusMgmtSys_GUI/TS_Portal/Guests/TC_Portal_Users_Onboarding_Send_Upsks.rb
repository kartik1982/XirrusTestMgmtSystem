...
###########################################################################################################################################
###################TEST CASE: PORTALS area - Test the ONBOARDING PORTAL - USERS TAB - ADD SEVERAL USERS SEND U-PSKs TO SOME OF THEM#########
###########################################################################################################################################
describe  "********** TEST CASE: PORTALS area - Test the ONBOARDING PORTAL - USERS TAB - ADD SEVERAL USERS SEND U-PSKs TO SOME OF THEM **********" do
  ...
  used_user_name_even_only = []

  used_user_names.each_with_index do |user_name, i|
    if i.even? == true
      used_user_name_even_only << user_name
    end
  end

  include_examples "send upsk", portal_name, used_user_name_even_only, @used_user_emails

  used_user_name_even_only.each do |user_name|
      needed_index = users_hash["User Names"].index(user_name)
      user_email = users_hash["User Emails"][needed_index]
