...
##############################################################################################################
##############TEST CASE: PORTAL - COMPOSITE - ADD, DUPLICATE AND DELETE PORTALS IN UI#########################
##############################################################################################################
describe "********** TEST CASE: PORTAL - COMPOSITE - ADD, DUPLICATE AND DELETE PORTALS IN UI  **********" do # Creted on 15.05.2017

  portal_from_tile_name = "COMPOSITE - " + UTIL.ickey_shuffle(6) + " - CREATED FROM EMPTY GRID TILE"
  portal_from_btn_name = "COMPOSITE - " + UTIL.ickey_shuffle(6) + " - CREATED FROM NEW PORTAL BUTTON"
  portal_from_header_name = "COMPOSITE - " + UTIL.ickey_shuffle(6) + " - CREATED FROM HEADER MENU DROPDOWN"
  ...
  2.times do |i|
  	include_examples "create portal from header menu", "SELF REGISTRATION portal #{i}", "DESCRIPTION TEXT", 'self_reg'
  end
