...
describe "*******TESTCASE: PUBLIC API FOR EASYPASS ************" do
  ...
  voucher_portal ={name: "Public_API_Voucher_Portal", description: "Description for Public_API_Voucher_Portal", type: "VOUCHER"}
  voucher_load = {accessCode: "kartikfreecode"}
  voucher=nil
  before :all do    
     @papi= public_api
     if @env != "preview"
  ...
  it "verify public API to add specific number of vouchers to easypass portal" do
    response = @papi.post_add_specific_num_voucher_to_easypass_portal(voucher_portal[:name], 10)
    expect(response.code).to eq(200)
    expect(JSON.parse(response.body)).to eq("Vouchers have been added")
  end
  it "verify public API to add vouchers to easypass portal" do
    response = @papi.post_add_vouchers_to_easypass_portal(voucher_portal[:name], voucher_load)
    voucher = JSON.parse(response.body)
    expect(response.code).to eq(200)
    expect(voucher['accessCode']).to eq(voucher_load[:accessCode])
  end
  it "verify public API to update voucher for easypass portal" do
    voucher.update({'note'=> "api testing note", 'accessTerminationDate'=> DateTime.now.next_day(2).to_time.to_i})
    response = @papi.put_update_voucher_to_easypass_portal(voucher_portal[:name], voucher_load[:accessCode], voucher)
    voucher = JSON.parse(response.body)
    expect(response.code).to eq(200)
    expect(voucher['note']).to eq("api testing note")
  end
  it "verify public API to get voucher for easypass portal with access code" do
    response = @papi.get_voucher_from_easypass_portal_with_access_code(voucher_portal[:name], voucher_load[:accessCode])
    voucher = JSON.parse(response.body)
    expect(response.code).to eq(200)
    expect(voucher['note']).to eq("api testing note")
  end
  it "verify public API to get vouchers for easypass portal" do
    response = @papi.get_vouchers_from_easypass_portal(voucher_portal[:name])
    vouchers = JSON.parse(response.body)
    expect(response.code).to eq(200)
    expect(vouchers.size).to be > 10
  end
  it "verify public API to delete voucher form easypass portal with access code" do
    response = @papi.delete_voucher_from_easypass_portal_with_access_code(voucher_portal[:name], voucher_load[:accessCode])
    expect(response.code).to eq(200)
    expect(JSON.parse(response.body)).to eq("Voucher deleted")
  end
  ...
  it "delete all vouchers from portal" do
    vouchers = JSON.parse(@papi.get_vouchers_from_easypass_portal(voucher_portal[:name]).body)
    vouchers.each do |vouch|
      @papi.delete_voucher_from_easypass_portal_with_access_code(voucher_portal[:name], vouch['accessCode'])
    end
  end
end